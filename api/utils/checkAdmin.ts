...
export async function isRequestUserAdmin (req: Request, res: Response) : Promise<boolean> {
    ...
    if (!idUser) {
        res.status(400).json({error: "Formato de request equivocado. Falta idUser."})
        return false;
    };

    const isAdmin = await checkUserIsAdmin(idUser);
    if (!isAdmin) {
        res.status(403).json({ error: "Usuario no tiene permisos suficientes para acceder a la ruta" });
        return false;
    }
    return true;
}